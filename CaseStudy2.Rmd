...
```{r}
  turnoverData %>% ggplot(aes(x=DistanceFromHome)) + geom_bar()
```

## Predictions of Attrition
I set up a point system to make the data more KNN friendly.  I chose the top factors and assigned them as Personal Points, or Business Points. If an employee has the Reason and Factor in this list, they get a point added, else they get 0. More points means an increased chance that Attrition will equal "Yes".
...
JobInvolvement	3	
JobLevel	1	
JobRole	Sales Executive	or Research Scientist
OverTime	Yes	
PerformanceRating	3	
StockOptionLevel	0	
TrainingTimesLastYear	2	
YearsInCurrentRole	0 or	2
...
attritionChart = turnoverPoints %>% ggplot(aes(x=PersonalPoints, y = BusinessPoints, fill = Attrition)) + geom_col(position="dodge2") + coord_flip()

ggplotly(attritionChart, height=1000)
...
for (i in attritionTest[1]) {
  ...
  df1knn = knn(attritionTrain[,c(2,3)], df1, attritionTrain$Attrition, k = 3, prob = TRUE)
}
